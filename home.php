...
<div class="relative overflow-hidden">
	...
	<div class="relative flex items-center justify-center container mx-auto px-12 mt-20">
		...
		<div class="relative">
			<span class="ben-div-ref bg-white px-4 py-1.5 rounded-md text-[11px] font-bold tracking-[0.2em] uppercase text-gray-400 border border-gray-100 shadow-sm">
				<?php echo esc_html( get_theme_mod( 'ben_divider_text', 'Nøkkelfordeler' ) ); ?>
			</span>
		</div>
	</div>
...
<section class="relative container mx-auto px-4 md:px-12 py-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-16">
        <h2 class="ben-title-ref reveal-type text-5xl font-serif leading-tight text-gray-900">
            <?php echo esc_html( get_theme_mod( 'ben_main_title', 'Planlegging som gir mer tid...' ) ); ?>
        </h2>
        <div class="reveal-text lg:pl-12 lg:border-l border-gray-200">
            <p class="ben-desc-ref text-gray-500 leading-relaxed text-sm">
                <?php echo esc_html( get_theme_mod( 'ben_desc', 'Hjemmetjenesten er kompleks...' ) ); ?>
            </p>
        </div>
    </div>
    ...
    <div class="reveal-grid grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="reveal-card lg:col-span-1 lg:row-span-2 bg-white rounded-[32px] p-4 md:p-8 shadow-2xl shadow-gray-200 border border-gray-50 flex flex-col h-full">
            <div class="bg-[#EEF2FF] rounded-2xl h-80 w-full mb-8 relative overflow-hidden border border-blue-50">
                <?php 
                $card1_img = get_theme_mod('ben_card_1_img', get_template_directory_uri() . '/images/benefit-first-block.svg'); 
                ?>
                <img src="<?php echo esc_url($card1_img); ?>" alt="Map route" class="w-full h-full object-cover opacity-60" />
                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-2 rounded-lg shadow-md text-[10px] font-bold">Pasient</div>
            </div>
            ...
            <h3 class="ben-c1-t text-2xl font-serif mb-4"><?php echo esc_html(get_theme_mod('ben_card_1_title')); ?></h3>
            <div class="w-24 h-[1px] bg-gray-200 mb-6"></div>
            <p class="ben-c1-d text-sm text-gray-500 leading-relaxed mb-8"><?php echo esc_html(get_theme_mod('ben_card_1_desc')); ?></p>
            <button class="mt-auto flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-gray-400 bg-[#F9F5F3] w-fit px-4 py-2 rounded-full hover:bg-gray-100 transition-colors">
                <span class="bg-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] shadow-sm">›</span> Les mer
            </button>
        </div>

        <?php for($i=2; $i<=5; $i++): ?>
        <div class="reveal-card bg-white rounded-[32px] p-4 md:p-8 shadow-2xl shadow-gray-200 border border-gray-50 flex flex-col">
            <div class="bg-[#F9F5F3] w-10 h-10 rounded-xl flex items-center justify-center mb-6">
                <?php if($i == 2): ?>
                ...
                <?php else: ?>
                    <span class="text-gray-400 text-xl font-bold italic">✱</span>
                <?php endif; ?>
            </div>
            <h3 class="ben-c<?php echo $i; ?>-t text-xl font-serif mb-4 leading-tight"><?php echo esc_html(get_theme_mod("ben_card_{$i}_title")); ?></h3>
			<div class="w-24 h-[1px] bg-gray-200 mb-6"></div>
            <p class="ben-c<?php echo $i; ?>-d text-xs text-gray-500 leading-relaxed mb-8"><?php echo esc_html(get_theme_mod("ben_card_{$i}_desc")); ?></p>
            <button class="mt-auto flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-gray-400 bg-[#F9F5F3] w-fit px-4 py-2 rounded-full">
                <span class="bg-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] shadow-sm">›</span> Les mer
            </button>
        </div>
        <?php endfor; ?>
	...
	<?php
// Prepare the data for Alpine.js
$func_categories = array('Optimering', 'Fleksibilitet', 'Integrasjon', 'Brukervennlighet');
$tags_data = array();

foreach ($func_categories as $cat) {
    $slug = strtolower($cat);
    $raw_tags = get_theme_mod("func_tags_$slug", "Eksempel 1\nEksempel 2");
    // Convert newline separated string to array
    $tags_data[$cat] = array_filter(array_map('trim', explode("\n", $raw_tags)));
}
?>

<section x-data='{ 
    activeTab: "<?php echo $func_categories[0]; ?>", 
    categories: <?php echo json_encode($func_categories); ?>,
    tags: <?php echo json_encode($tags_data); ?>
}' class="container mx-auto px-6 py-16 text-center">
    
    <div class="mb-12">
        <span class="func-badge-ref bg-[#E8E2E1] italic px-6 py-3 rounded-md text-[11px] tracking-[0.2em] shadow-sm inline-block">
            <?php echo esc_html( get_theme_mod('func_badge', 'Hvorfor SPIDER?') ); ?>
        </span>
    </div>

    <h2 class="func-title-ref reveal-type text-3xl md:text-5xl font-serif text-gray-900 mb-8 md:mt-6">
        <?php echo esc_html( get_theme_mod('func_title', 'Funksjonalitet uten grenser') ); ?>
    </h2>

    <div class="reveal-text">
        <p class="func-desc-ref max-w-3xl mx-auto text-gray-500 leading-relaxed mb-16 text-sm">
            <?php echo esc_html( get_theme_mod('func_desc', 'SPIDER er utviklet tett sammen...') ); ?>
        </p>
    </div>
    ...
    <div class="flex flex-col items-center gap-8">
        ...
        <div class="flex items-center gap-6">
            ...
            <span class="func-bottom-badge-ref bg-[#EAE8E4] px-6 py-2 rounded-lg text-[12px] font-serif italic text-gray-700 tracking-wide">
                <?php echo esc_html( get_theme_mod('func_bottom_badge', 'SPIDER er unik på flere områder') ); ?>
            </span>
            <div class="flex gap-1.5">
                <div class="w-1.5 h-1.5 rotate-45 border border-gray-300"></div>
                <div class="w-1.5 h-1.5 rotate-45 bg-gray-300"></div>
